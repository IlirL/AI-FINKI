...
# from Test2.Problem1 import astar_search
...
class Problem:
    ...
    def successor(self, state):
        ...
        that yields the successors one at a time, rather tha    n building them
        all at once.
...
def validState(crveniJabolki, snakePosition):
    head = snakePosition[0]
    i, j = head

    if i < 0 or i >= 10 or j < 0 or j >= 10:
        return False
    if head in crveniJabolki:
        return False
    return True


def hitYourself(head, snakePosition):
    if head in snakePosition:
        return True
    return False


def prodolzhiPravo(snakePosition, crveniJabolki, zeleniJabolkiPozicii, direction):
    new_snake_position = []
    head = snakePosition[0]
    tail = snakePosition[len(snakePosition) - 1]
    returnedTuple = None
    if direction == "south":
        new_head = (head[0], head[1] - 1)
        if hitYourself(new_head, snakePosition) == False:

            new_snake_position.append(new_head)
            new_zeleniJabolkiPozicii = []

            if validState(crveniJabolki, new_snake_position):
                for i in range(0, len(snakePosition) - 1):
                    new_snake_position.append(snakePosition[i])

                currentLengthOfZeleniJabolki = len(zeleniJabolkiPozicii)
                new_zeleniJabolkiPozicii = tuple(
                    (i, j) for (i, j) in zeleniJabolkiPozicii if (i != new_head[0] or j != new_head[1]))
                afterLengthOfZeleniJabolki = len(new_zeleniJabolkiPozicii)
                if currentLengthOfZeleniJabolki != afterLengthOfZeleniJabolki:
                    new_snake_position.append(tail)
                returnedTuple = tuple((tuple(new_snake_position), new_zeleniJabolkiPozicii, "south"))
    ...
    if direction == "north":
        ...
        if hitYourself(new_head, snakePosition) == False:
            ...
            if validState(crveniJabolki, new_snake_position):
                ...
                if currentLengthOfZeleniJabolki != afterLengthOfZeleniJabolki:
                    new_snake_position.append(tail)
                returnedTuple = tuple((tuple(new_snake_position), new_zeleniJabolkiPozicii, "north"))
                return returnedTuple
    ...
    if direction == "east":
        ...
        if hitYourself(new_head, snakePosition) == False:
            ...
            if validState(crveniJabolki, new_snake_position):
                ...
                if currentLengthOfZeleniJabolki != afterLengthOfZeleniJabolki:
                    new_snake_position.append(tail)
                returnedTuple = tuple((tuple(new_snake_position), new_zeleniJabolkiPozicii, "east"))
                return returnedTuple
    ...
    if direction == "west":
        ...
        if hitYourself(new_head, snakePosition) == False:
            ...
            if validState(crveniJabolki, new_snake_position):
                ...
                if currentLengthOfZeleniJabolki != afterLengthOfZeleniJabolki:
                    new_snake_position.append(tail)
                returnedTuple = tuple((tuple(new_snake_position), new_zeleniJabolkiPozicii, "west"))
                return returnedTuple
    return returnedTuple


def svrtiDesno(snakePosition, crveniJabolki, zeleniJabolkiPozicii, direction):
    new_snake_position = []
    head = snakePosition[0]
    tail = snakePosition[len(snakePosition) - 1]
    returnedTuple = None
...
def svrtiLevo(snakePosition, crveniJabolki, zeleniJabolkiPozicii, direction):
    new_snake_position = []
    head = snakePosition[0]
    tail = snakePosition[len(snakePosition) - 1]
    returnedTuple = None
...
class Snake(Problem):
    def __init__(self, crveniJabolki, initialState, goal=None):
        super().__init__(initialState, goal)
        self.crveniJabolki = crveniJabolki

    def successor(self, state):
        successors = dict()

        snakePosition = state[0]
        zeleniJabolkiPozicii = state[1]
        direction = state[2]

        prodolzhiPravoTuple = prodolzhiPravo(snakePosition, self.crveniJabolki, zeleniJabolkiPozicii, direction)
        svrtiDesnoTuple = svrtiDesno(snakePosition, self.crveniJabolki, zeleniJabolkiPozicii, direction)
        svrtiLevoTuple = svrtiLevo(snakePosition, self.crveniJabolki, zeleniJabolkiPozicii, direction)

        if prodolzhiPravoTuple != None:
            successors["ProdolzhiPravo"] = prodolzhiPravoTuple
        if svrtiDesnoTuple != None:
            successors["SvrtiDesno"] = svrtiDesnoTuple
        if svrtiLevoTuple != None:
            successors["SvrtiLevo"] = svrtiLevoTuple
...
if __name__ == '__main__':
    ...
    numberOfZeleniJabolki = int(input())
    zeleniJabolkiPozicii = []

    for i in range(numberOfZeleniJabolki):
        pozicija = input().split(',')
        zeleniJabolkiPozicii.append((int(pozicija[0]), int(pozicija[1])))

    numberOfCrveniJabolki = int(input())
    crveniJabolkiPozicii = []
    for i in range(numberOfCrveniJabolki):
        pozicija = input().split(',')
        crveniJabolkiPozicii.append((int(pozicija[0]), int(pozicija[1])))

    snakeOpfakjaPole = [(0, 7), (0, 8), (0, 9)]

    initialState = (tuple(snakeOpfakjaPole), tuple(zeleniJabolkiPozicii), "south")
    snake = Snake(tuple(crveniJabolkiPozicii), initialState)
