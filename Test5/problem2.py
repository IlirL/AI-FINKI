...
if __name__ == '__main__':
    ...
    br_drva = int(input())
    kriteriumm = input()
    zapis = [e for e in input().split(" ")]

    train_set = dataset[0:int(0.85*len(dataset))]
    train_set_karakteristiki = [t[:-1] for t in train_set]
    train_set_klasi = [t[-1] for t in train_set]

    test_set = dataset[int(0.85 * len(dataset))]
    test_set_karakteristiki = [t[:-1] for t in test_set]
    test_set_klasi = [t[-1] for t in test_set]

    train_karakteristiki2 = list()

    for t in test_set_karakteristiki:
        sample = [t[i] for i in range(0, len(t)) if i!= kolona]
        train_karakteristiki2.append(sample)


    test_karakteristiki2 = list()
    for t in test_set_karakteristiki:
        sample = [t[i] for i in range(0, len(t)) if i!=kolona]
        test_karakteristiki2.append(sample)

    if kriteriumm == 'gini':
        tree1 = RandomForestClassifier(n_estimators= br_drva, criterion = 'gini', random_state = 0)
    else:
        tree1 = RandomForestClassifier(n_estimators = br_drva, criterion = 'entropy', random_state = 0)

    tree1.fit(train_karakteristiki2, train_set_klasi)

    correct_predictions = 0
    for x,y in zip(test_karakteristiki2, test_set_klasi):
        prediction = tree1.predict([x])
        if y == prediction:
            correct_predictions+=1

    print("Accuracy:", correct_predictions/len(test_set))

    zapis2 = [zapis[i] for i in range(0, len(zapis)) if i!=kolona]
    prediction = tree1.predict([zapis2])
    print(str(prediction[0]))
    proba = tree1.predict_proba([zapis2])
    print(proba[0])