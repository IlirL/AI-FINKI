...
if __name__ == '__main__':
    ...
    col_index = int(input())
    brojot_na_drva = int(input())
    criterion = input()
    test_from_input = [float(el) for el in input().split(' ')]
    test_from_input = [test_from_input[i] for i in range(len(test_from_input)) if i!=col_index]
    dataset = [[dataset[i][j] for j in range(len(dataset[i])) if j != col_index] for i in range(len(dataset))]
    ...
    if(criterion == 'entropy'):
        classifier = RandomForestClassifier(n_estimators = brojot_na_drva, criterion = 'entropy', random_state = 0)
    else:
        classifier = RandomForestClassifier(n_estimators = brojot_na_drva, criterion = 'gini', random_state = 0)
    ...
    prediction_from_input = classifier.predict([test_from_input])[0]
    # print(accuracy)
    print(f'Accuracy: {accuracy}')
    print(prediction_from_input)
    print(classifier.predict_proba([test_from_input])[0])