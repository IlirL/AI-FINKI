...
class Squares(Problem):
    ...
    def h(self, node):
        ...
        for i in range(len(state)):
            #We have the fucking pair in i
            #We use the heuristics at which we see how far away is each of the square from the goal and add them
            value = value + abs(state[i][0] - self.goal[i][0]) + abs(state[i][1] - self.goal[i][1])
