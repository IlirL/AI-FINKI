...
class Disks(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            for j in range(len(state)):
                #j is also a tuple
                if len(state[i])>0 and i!=j:
                    print("Trying to move pillar " + str(i) + " to pillar "+str(j))
                    currentState = []
                    newTuplei = state[i]
                    newTuplei = list(state[i])
                    elementPopped = newTuplei.pop();
                    newTuplei = tuple(newTuplei)

                    print("newTuplei = ", newTuplei)

                    newTuplej = state[j]
                    newTuplej = list(newTuplej)
                    newTuplej.append(elementPopped)
                    newTuplej = tuple(newTuplej)

                    print("newTuplej = ", newTuplej)
                    for k in range(len(state)):
                        if k!=i and k!=j:
                            currentState.append(state[k])
                        elif k==i:
                            currentState.append(newTuplei)
                        else:#meaning k==j
                           currentState.append(newTuplej)


                    print("In the end currentState = ", currentState)
                    successors["MOVE TOP BLOCK FROM PILLAR " + str(i) + " TO PILLAR " + str(j)] = tuple(currentState)
            return successors
    def actions(self, state):
    ...
    def goal_test(self, state):
        print("in goal state: goalState = ", goalState)
...
if __name__ == "__main__":
    ...
    initialState = []
    towersInitial = line1.split(";")
    # towerWithAllDisksInitial = []
    for singleString in towersInitial:
        if(len(singleString) > 0):
            initialState.append(tuple(singleString.split(",")))
        else:
            initialState.append(tuple())
    initialState = tuple(initialState)
    # print(initialState)
    towersGoal = line2.split(";")
    goalState = []
    for singleString in towersGoal:
        if(len(singleString) > 0):
            goalState.append(tuple(singleString.split(",")))
        else:
            goalState.append(tuple())
    goalState = tuple(goalState)

    # print(initialState)
    # print(goalState)

    disks = Disks(initialState,goalState)
